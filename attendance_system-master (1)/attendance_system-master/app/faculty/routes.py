...
@router.get("/")
def get_faculties_route(session: Session = Depends(get_session)):
    return get_faculties(session)

@router.put("/{faculty_id}")
def update_faculty_route(faculty_id: int, new_name: str, session: Session = Depends(get_session)):
...
def delete_faculty_route(faculty_id: int, session: Session = Depends(get_session)):
